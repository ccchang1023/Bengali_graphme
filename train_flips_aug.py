...
def get_resnext_model(model_type="101", pretrained=True, dropout=None):
    ...
    model.classifier_root = nn.Linear(model.feature_dim, 168*FLIP_COEF)
    model.classifier_vowel = nn.Linear(model.feature_dim, 11*FLIP_COEF)
    model.classifier_constant = nn.Linear(model.feature_dim, 7*FLIP_COEF)
    return model
...
class BengaliDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img0 = self.trans_tensor(img)
        label0 = torch.as_tensor(labels, dtype=torch.int16)

        if self.is_validate or FLIP_COEF == 1:
            return img0, label0

        if FLIP_COEF >= 2:
            if FLIP_TYPE.find("H")!=-1:
                img1 = self.trans_hflip(img)
            elif FLIP_TYPE.find("V")!=-1:
                img1 = self.trans_vflip(img)
            else:
                print("Wrong FLIP TYPE")
            img1 = self.trans_tensor(img1)
            label1 = label0.clone().detach()
            label1[0] += torch.tensor(168,dtype=torch.int16)
            label1[1] += torch.tensor(11,dtype=torch.int16)
            label1[2] += torch.tensor(7,dtype=torch.int16)
            return img0,img1,label0,label1

        if FLIP_COEF >= 3:
            img2 = self.trans_vflip(img)
            img2 = self.trans_tensor(img2)
            label2 = label0.clone().detach()
            label2[0] += torch.tensor(168*2,dtype=torch.int16)
            label2[1] += torch.tensor(11*2,dtype=torch.int16)
            label2[2] += torch.tensor(7*2,dtype=torch.int16)
            return img0,img1,img2,label0,label1,label2
...
if __name__ == "__main__":
    ...
    save_file_name = "./B_saved_model_0226/4fold_ocp0.15_prcnt15_div50_EP180_Tune10_b200_vp640_224x224_pre1_way1Mor0.3Cutmix0.8Cutout0.2_LS_fp16_fold{}".format(FOLD)
    ...
    for fold in range(0,len(train_loaders)):
        ...
        for ep in range(0,epochs+1+FINE_TUNE_EP):
            ...
            for idx, data in enumerate(train_loader):
                ...
                ###Flip coef == 2 or 3
                img,img2,target,target2 = data
                img, target = img.to(device), target.to(device,dtype=torch.long)
                img2, target2 = img2.to(device), target2.to(device,dtype=torch.long)
                img_list = [img,img2]
                target_list = [target,target2]
                for flip_i in range(len(img_list)):
                    img = img_list[flip_i]
                    target = target_list[flip_i]
                ...
                if idx!=0 and idx%val_period == 0:
                    ...
                    with torch.no_grad():
                        for idx, data in enumerate(val_loader):
                            ...
                            if FLIP_COEF >= 2:
                                # print("here",np.shape(pred_root))
                                # print(pred_root[0,:3],pred_root[0,168:171])
                                pred_root[:,:168] = pred_root[:,:168]/FLIP_COEF + pred_root[:,168:]/FLIP_COEF
                                pred_root = pred_root[:,:168]
                                # print("here2",np.shape(pred_root))
                                # print(pred_root[0,:3])
                                
                                pred_vowel[:,:11] = pred_vowel[:,:11]/FLIP_COEF + pred_vowel[:,11:]/FLIP_COEF
                                pred_vowel = pred_vowel[:,:11]

                                pred_constant[:,:7] = pred_constant[:,:7]/FLIP_COEF + pred_constant[:,7:]/FLIP_COEF
                                pred_constant = pred_constant[:,:7]
