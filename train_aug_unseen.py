...
# train_images = np.load("./0310_unseen_imgs.npy")
# train_labels = np.load("./0310_unseen_labels.npy")
...
def get_unseen_dataset_loader(val_rate=0.1,indices_len=None, batch_size=None,num_workers=None):
    train_loader_list = []
    val_loader_list = []
    # idx = indices_len*(1-val_rate)
    indices_train = []
    indices_val = []

    ###5% unseen
    fold_split = [[300,360],[500,560],[900,960],[1100,1160],[1200,1260]]

    ###10% unseen
    # fold_split = [[300,420],[500,620],[700,820],[900,1020],[1100,1220]]

    bottom = fold_split[FOLD][0]
    top = fold_split[FOLD][1]

    for i in range(indices_len):
        mod_i = i%1292
        if mod_i>=bottom and mod_i<top:
            indices_val.append(i)
        else:
            indices_train.append(i)

    # np.random.shuffle(indices_train)
    ind = indices_train + indices_val
    # print("train:",ind[:10])
    # print("val:",ind[-20:])
...
if __name__ == "__main__":
    ...
    lr = 1e-5
    batch_size = 128
    val_period = 30000        #only val at every ep end
    ...
    # indices_len = 206720
    vr = 1/k     
    # vr = 0.1    ### 5% unseen(60*160=9600) + 5% random seen(~10000)
    # vr = 0.15    ### 10% unseen(120*160=19200) + 5% random seen(~10000)

    print("validation rate:",vr)
    train_loaders, val_loaders = get_kfold_dataset_loader(k, vr, indices_len, batch_size, num_workers)
    # train_loaders, val_loaders = get_unseen_dataset_loader(vr, indices_len, batch_size, num_workers)

    save_file_name = "./B_saved_model_0313_final/large_10fold_ocp0.15_prcnt15_div50_EP200_Tune10_b128_vpX_224x224_pre1_Mor0.2uniCutmix1_CBL_noLS_fp16_fold{}".format(FOLD)
...
#         lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer,T_0=period,T_mult=1,eta_min=1e-5) #original 
        ...
        for ep in range(0,epochs+1+FINE_TUNE_EP):
            ...
            for idx, data in enumerate(train_loader):
                ...
                # axes[0][0].imshow(img[0][0].cpu().numpy())
                # axes[0][1].imshow(img[batch_size-1][0].cpu().numpy())
                ...
                # axes[1][0].imshow(img[0][0].cpu().numpy())
                # axes[1][1].imshow(img[batch_size-1][0].cpu().numpy())
                # plt.pause(.1)
                # input('stop')
